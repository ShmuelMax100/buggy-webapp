...
// FIXED: Removed `catalog` field and directly used `db.products`.
const db = {
  products: [
...
// ── FIXED: /api/products → Correctly accesses `db.products` ───────────────────
app.get('/api/products', (_req, res) => {
  try {
    const items = db.products;   // Correct access
...
app.listen(PORT, () => {
  ...
  console.log('  GET /api/products → FIXED: No more 500 Internal Server Error');
});